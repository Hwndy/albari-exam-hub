...
export const TeacherExamBuilder: React.FC = () => {
  ...
  const fetchExams = async () => {
    try {
      setLoading(true);
      // Only fetch exams created by this teacher and restricted to their subjects/classes
      ...
      // If teacher has specific assignments, filter exams
  ...
  const handleDeleteExam = async (examId: string, examTitle: string) => {
    ...
    try {
      const { error } = await supabase
        ...
        .delete()
        .eq('id', examId)
        .eq('created_by', user?.id); // Only allow deletion of own exams
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>My Exams ({filteredExams.length})</CardTitle>
        </CardHeader>
        <CardContent>
