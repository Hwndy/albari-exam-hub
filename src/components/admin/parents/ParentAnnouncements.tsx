...
interface Announcement {
  ...
  is_published: boolean; target_audience: string[]; created_at: string;
}
...
export const ParentAnnouncements: React.FC = () => {
  ...
  const [form, setForm] = useState({ title: '', content: '', priority: 'normal', audience: 'parent' as 'parent' | 'all' });
  const [saving, setSaving] = useState(false);

  const load = async () => {
    setLoading(true);
    const { data } = await supabase.from('announcements')
      .select('id,title,content,priority,is_published,target_audience,created_at')
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('announcements').insert({
        ...
        priority: form.priority, target_audience: [form.audience],
        is_published: true, publish_date: new Date().toISOString(),
        created_by: user?.id,
      });
      if (error) throw error;
      toast({ title: 'Announcement published' });
      setForm({ title: '', content: '', priority: 'normal', audience: 'parent' });
      load();
    } catch (e: any) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card>
        <CardHeader><CardTitle>New parent announcement</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={submit} className="space-y-3">
            ...
            <Button type="submit" disabled={saving}>{saving ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Send className="h-4 w-4 mr-2" />} Publish</Button>
          </form>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">No parent-facing announcements yet.</p>
          ) : (
