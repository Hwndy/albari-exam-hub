...
import { FeeReceiptView } from "@/components/fees/FeeReceiptView";
import { printReceiptView, downloadReceiptView, ReceiptViewData } from "@/lib/receipt-print";
...
export const FeeReceiptGenerator = () => {
  ...
  const receiptData = (payment: Payment): ReceiptViewData => ({
    title: "FEE RECEIPT",
  ...
  const handleDownload = async (payment: Payment) => {
    await downloadReceiptView(receiptData(payment), `receipt-${payment.receipt_number || payment.id}.pdf`);
    toast.success("Receipt downloaded successfully");
  };

  const handlePrint = async (payment: Payment) => {
    await printReceiptView(receiptData(payment), payment.receipt_number || "Receipt");
  };
    ...
    <div className="space-y-6">
      ...
      <Dialog open={showPreviewDialog} onOpenChange={setShowPreviewDialog}>
        <DialogContent className="max-w-2xl">
          ...
          {selectedPayment && (
            <div ref={receiptRef}>
              <FeeReceiptView {...receiptData(selectedPayment)} school={schoolInfo} />
            </div>
          )}
