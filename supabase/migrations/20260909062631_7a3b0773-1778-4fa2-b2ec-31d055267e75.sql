...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
           'name', q.name, 'order', q.level_order, 'male', q.male, 'female', q.female
         ) ORDER BY q.level_order), '[]'::jsonb)
  INTO v_by_level
  FROM (
    SELECT cl.name AS name, cl.level_order AS level_order,
           count(*) FILTER (WHERE lower(coalesce(s.gender,'')) = 'male') AS male,
           count(*) FILTER (WHERE lower(coalesce(s.gender,'')) = 'female') AS female
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object('date', q.d, 'rate', q.rate) ORDER BY q.d), '[]'::jsonb)
  INTO v_att_trend
  FROM (
    SELECT ses.date AS d,
           round(100.0 * count(*) FILTER (WHERE sa.status IN ('present','late')) / nullif(count(*),0), 1) AS rate
    FROM public.student_attendance sa
    JOIN public.attendance_sessions ses ON ses.id = sa.attendance_session_id
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
           ...
           'label', to_char(q.m, 'Mon'),
           'amount', q.amt
         ) ORDER BY q.m), '[]'::jsonb)
  INTO v_fee_trend
  FROM (
    SELECT date_trunc('month', coalesce(p.payment_date, p.created_at::date)) AS m,
           sum(p.amount_paid) AS amt
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object('status', q.st, 'count', q.c)), '[]'::jsonb)
  INTO v_funnel
  FROM (
